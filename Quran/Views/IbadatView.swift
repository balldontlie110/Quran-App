...
struct IbadatView: View {
    ...
    var body: some View {
        ScrollView {
            LazyVStack {
                ForEach(orderedIbadat) { ibadah in
                    ...
                    Button {
                        ...
                        if editMode && isFavorite {
                            IbadahCard(ignoreNavigationGesture: $ignoreNavigationGesture, ibadah: ibadah, isFavorite: isFavorite, editMode: $editMode) {
...
struct IbadahCard: View {
    ...
    var body: some View {
        HStack(spacing: 15) {
            if let id = ibadah.dua?.id ?? ibadah.ziyarat?.id ?? ibadah.amaal?.id {
                Text(String(id))
                    .bold()
                    .overlay {
